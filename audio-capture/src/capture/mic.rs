...
/// Number of frames to accumulate per chunk sent downstream.
/// At 16kHz, 1600 frames = 100ms.
const CHUNK_FRAMES: usize = 1600;
...
pub fn run(tx: Sender<AudioChunk>, _chunk_ms: u32) -> Result<()> {
    ...
    // Request 32-bit float, native sample rate, stereo (autoconvert handles channel mismatch)
    ...
    let target_samples = CHUNK_FRAMES;
    ...
    loop {
        ...
        // Drain queue into float samples
        while sample_queue.len() >= blockalign {
            ...
            match resampler.process(&interleaved) {
                ...
                Err(e) => {
                    warn!("Mic resample error: {e}");
                }
            }
        }

        while accumulated.len() >= target_samples {
            let chunk_data: Vec<f32> = accumulated.drain(..target_samples).collect();
